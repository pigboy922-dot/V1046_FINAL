...
try:
    ...
    read_worksheet_records = None
    verify_cloud_sync = None
...
def cloud_readonly() -> bool:
    return env_bool("V1046_CLOUD_READONLY", True)
...
def cloud_bar_html() -> str:
    ...
    auto = "｜本頁只讀 Google 雲端；本機負責運算"
    return f"""
<div id="v1046-cloudbar" data-status="idle">
  <strong>{APP_NAME}</strong>
  <a class="v1046-primary" href="/?auto=0">雲端看板</a>
  <a href="/?auto=0&reload=1">從 Google 重新載入</a>
  <a href="{url_for('health_full')}">完整健康檢查</a>
  <a href="{url_for('api_status')}">API 狀態</a>
  <span class="v1046-status"><span class="v1046-dot"></span><span id="v1046-cloud-status-text">雲端看板已載入{auto}{locked}</span></span>
</div>
"""
...
(function(){
  ...
  async function poll(){
    try{
      ...
      if(s.google_sheets && s.google_sheets.configured) bits.push('Sheets OK');
      if(s.google_drive && s.google_drive.configured) bits.push('Drive OK');
      if(s.auto_open_last_skipped_reason) bits.push(s.auto_open_last_skipped_reason);
      text.textContent = bits.join('｜') || '雲端看板';
...
def _records_table(records, columns, empty_msg="尚無雲端資料") -> str:
    if not records:
        return f"<div class='empty'>{empty_msg}</div>"
    thead = "".join(f"<th>{escape(label)}</th>" for key, label in columns)
    body = []
    for r in records[:80]:
        cells = "".join(f"<td>{_fmt_cell(r.get(key, ''))}</td>" for key, label in columns)
        body.append(f"<tr>{cells}</tr>")
    return f"<table><thead><tr>{thead}</tr></thead><tbody>{''.join(body)}</tbody></table>"


def _cloud_read(sheet_name: str, limit: int = 100):
    if read_worksheet_records is None:
        return {"ok": False, "records": [], "error": "read_worksheet_records unavailable"}
    return read_worksheet_records(sheet_name, limit=limit)


def cloud_board_page() -> str:
    sheets = get_sheets_status(light=False) if get_sheets_status is not None else {"enabled": False, "configured": False, "connect_ok": False}
    drive = get_drive_status(light=False) if get_drive_status is not None else {"enabled": False, "configured": False, "connect_ok": False}
    action = _cloud_read("output_today_action_summary", 20)
    recs = _cloud_read("output_today_recommendations", 80)
    pos = _cloud_read("state_positions", 80)
    closed = _cloud_read("state_closed_trades", 80)
    health = _cloud_read("output_health_log", 80)
    action_rows = action.get("records", [])
    rec_rows = recs.get("records", [])
    pos_rows = pos.get("records", [])
    closed_rows = closed.get("records", [])[-30:]
    health_rows = health.get("records", [])[-60:]
    stat = "OK" if sheets.get("connect_ok") and drive.get("connect_ok") else "WARN"
    msg = "本頁只讀 Google Sheets / Drive；重運算請在本機跑完同步雲端。"
    css = cloud_css()
    return f"""<!doctype html><html lang='zh-Hant'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>{APP_NAME}</title>{css}<style>
body{{margin:0;background:#eef2f6;color:#243447;font-family:'Microsoft JhengHei',Arial,sans-serif}}.wrap{{max-width:1180px;margin:0 auto;padding:18px}}.card{{background:#fffdf7;border:1px solid #dde4dc;border-radius:18px;margin:14px 0;padding:16px;box-shadow:0 8px 24px rgba(15,23,42,.06)}}h1{{margin:0 0 8px}}.sub{{color:#64748b;line-height:1.7}}table{{border-collapse:collapse;width:100%;font-size:13px}}th,td{{border-bottom:1px solid #e2e8d7;padding:8px;text-align:left;vertical-align:top}}th{{background:#eef1e6}}a{{color:#2563eb;font-weight:800}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px}}.metric{{background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:12px}}.metric b{{font-size:24px;display:block}}.ok{{color:#15803d;font-weight:900}}.warn{{color:#b45309;font-weight:900}}.empty{{padding:12px;color:#64748b;background:#f8fafc;border-radius:12px}}
</style></head><body>{cloud_bar_html()}<div class='wrap'>
<div class='card'><h1>{APP_NAME}</h1><div class='sub'>{msg}</div><p class='{ 'ok' if stat=='OK' else 'warn'}'>Cloud status: {stat}｜Sheets connect={sheets.get('connect_ok')}｜Drive connect={drive.get('connect_ok')}</p></div>
<div class='card'><div class='grid'><div class='metric'><b>{len(recs.get('records', []))}</b><span>雲端推薦列</span></div><div class='metric'><b>{len(pos.get('records', []))}</b><span>雲端持倉列</span></div><div class='metric'><b>{len(action.get('records', []))}</b><span>今日動作列</span></div><div class='metric'><b>{len(closed.get('records', []))}</b><span>已出場列</span></div></div></div>
<div class='card'><h2>今日動作摘要</h2>{_records_table(action_rows, [('market_label','市場'),('today_action','今日動作'),('new_buy_count','新買'),('open_positions','持倉'),('max_positions','最多'),('sell_due_count','待賣'),('risk_level','風控'),('guard_reason','原因')])}</div>
<div class='card'><h2>本週推薦</h2>{_records_table(rec_rows, [('market','市場'),('symbol','代號'),('name_zh','名稱'),('recommended_price','推薦價'),('target_position_amount','建議投入'),('estimated_shares','預估股數'),('stop_loss_price','停損價'),('recommendation_date','日期'),('action','操作'),('reason','理由')])}</div>
<div class='card'><h2>紙上持倉</h2>{_records_table(pos_rows, [('market','市場'),('symbol','代號'),('name_zh','名稱'),('entry_date','買進日'),('entry_price','買進價'),('latest_date','最新日'),('latest_price','最新價'),('status','狀態'),('planned_hold_bars','H')])}</div>
<div class='card'><h2>最近出場</h2>{_records_table(closed_rows, [('market','市場'),('symbol','代號'),('name_zh','名稱'),('entry_date','買進日'),('exit_date','出場日'),('realized_return_pct','報酬%'),('exit_reason','原因')])}</div>
<div class='card'><h2>雲端同步健康紀錄</h2>{_records_table(health_rows, [('generated_at','時間'),('level','等級'),('message','訊息')])}</div>
</div>{cloud_script()}</body></html>"""

def landing_page() -> str:
    return cloud_board_page()
...
def api_run():
    if cloud_readonly():
        return jsonify({"status":"disabled","message":"Render 雲端看板模式：請在本機運算並同步 Google"}), 403
    if not check_token():
